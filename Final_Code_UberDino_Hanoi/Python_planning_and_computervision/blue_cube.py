...
import cv2
import time
from detection import extract_blue, extract_yellow, extract_gray, extract_orange, extract_black
import pygame
...
class BlueCube:
    ...
    def img_process_rgb2canny(self, input_image):        
        ...
        #if self.show_flag:
        #    cv2.imshow("medianBlur", blue)
        #    cv2.waitKey(1)
        gf_time = time.time()
        blue = cv2.Canny(blue, 50, 150, apertureSize = 3)    
        #if self.show_flag:
        #    cv2.imshow("Canny", blue)
        #    cv2.waitKey(1)
    ...
    def get_poly_6_points(self, dark_with_poly,
        ...
        if self.show_flag:
            ...
            cv2.imshow("poly contours features blue", dark)
            cv2.waitKey(1)
        else:
            dark = self.origin_img.copy()
            for i in corners:
