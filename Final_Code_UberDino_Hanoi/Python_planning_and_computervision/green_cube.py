...
import cv2
import time
from detection import extract_green, extract_yellow, extract_gray, extract_orange, extract_black
import pygame
...
class GreenCube:
    ...
    def cross_point(self, line1_ab, line2_ab):
    ...
    def img_process_rgb2canny(self, input_image):        
        ...
        #if self.show_flag:
        #    cv2.imshow("medianBlur", green)
        #    cv2.waitKey(1)
        gf_time = time.time()
        green = cv2.Canny(green, 50, 150, apertureSize = 3)    
        #if self.show_flag:
        #    cv2.imshow("Canny", green)
        #    cv2.waitKey(1)
    ...
    def get_poly_6_points(self, dark_with_poly,
        ...
        if self.show_flag:
            ...
            mean_x, av_x, mean_y, av_y = int(mean(X)), int(stdev(X)), int(mean(Y)), int(stdev(Y))
            self.center_x, self.center_y = mean_x, mean_y

            #cv2.circle(dark, (mean_x, mean_y), 3, (0, 200, 200), -1)
            cv2.ellipse(dark, (mean_x, mean_y), (av_x, av_y), 0.0, 0.0, 360.0, (255, 0, 255), -1) 

            cv2.imshow("poly contours features green", dark)
            cv2.waitKey(1)
        else:
            dark = self.origin_img.copy()
            for i in corners:
